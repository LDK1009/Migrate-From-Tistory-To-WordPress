...
export async function POST(req: NextRequest) {
  try {
    ...
    formattedFormData.forEach((article) => {
...
function extractArticleTitle(htmlFile: File) {
  // 변경된 코드
...
function changeFileName(file: File, nextFileName: string) {
...
async function changeImageTagSrc(articleNumber: number, htmlFile: File) {
  // File 객체를 ArrayBuffer로 변환
  const arrayBuffer = await htmlFile.arrayBuffer();
...
////////// 워드프레스 게시글 작성 함수
async function createArticle({ wpUrl, wpId, applicationPassword, articleData, articleImages }: CreateArticleType) {
  try {
...
////////// 워드프레스 이미지 업로드 함수
async function uploadImageToWordPress(wpUrl: string, authHeader: string, imageFile: File) {
  try {
